...
import { ctrlState } from './tab-exp-viewtabexppanel-tab-exp-panel-state';
import { TabExpViewtabviewpanelTabViewPanel } from '@widgets/chart-data/tab-exp-viewtabviewpanel-tab-view-panel';
import { TabExpViewtabviewpanel2TabViewPanel } from '@widgets/chart-data/tab-exp-viewtabviewpanel2-tab-view-panel';
interface Props {
  name:string,
  parent: IParam;
  context: IContext;
  ...
  <div class="app-tab-exp-panel">
    <a-tabs :tabPosition="state.tabPosition" v-model:activeKey="state.activePanel" class="app-tab-exp-panel__tabs">
      <a-tab-pane key="tabviewpanel">
        <template #tab>
          <AppIconText text="图表实体编辑视图"/>
        </template>
        <TabExpViewtabviewpanelTabViewPanel
          name="tabviewpanel"
      ...
      <a-tab-pane key="tabviewpanel2">
        <template #tab>
          <AppIconText text="图表实体表格视图"/>
        </template>
        <TabExpViewtabviewpanel2TabViewPanel
          name="tabviewpanel2"
          :context="state.context"
          :viewParams="state.viewParams"
          :viewSubject="state.viewSubject"
