package doc.core.sample.mapper;

import java.util.List;
import org.apache.ibatis.annotations.*;
import java.util.Map;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.core.conditions.Wrapper;
import java.util.Map;
import org.apache.ibatis.annotations.Select;
import doc.core.sample.domain.KanbanData;
import doc.core.sample.filter.KanbanDataSearchContext;
import org.springframework.cache.annotation.CacheEvict;
import org.springframework.cache.annotation.Cacheable;
import java.io.Serializable;
import com.baomidou.mybatisplus.core.toolkit.Constants;
import com.baomidou.dynamic.datasource.annotation.DS;
import com.alibaba.fastjson.JSONObject;

public interface KanbanDataMapper extends BaseMapper<KanbanData> {


    Page<KanbanData> searchDefault(IPage page, @Param("ctx") KanbanDataSearchContext context, @Param("ew") Wrapper<KanbanData> wrapper);
    List<KanbanData> listDefault(@Param("ctx") KanbanDataSearchContext context, @Param("ew") Wrapper<KanbanData> wrapper);

    @Override
    KanbanData selectById(Serializable id);

    KanbanData selectEntity(KanbanData entity);

    List<KanbanData> selectEntities(@Param("list") List<KanbanData> list);

    @Override
    int insert(KanbanData entity);

    @Override
    int updateById(@Param(Constants.ENTITY) KanbanData entity);

    @Override
    int update(@Param(Constants.ENTITY) KanbanData entity, @Param("ew") Wrapper<KanbanData> updateWrapper);

    @Override
    int deleteById(Serializable id);

    /**
    * 自定义查询SQL
    * @param sql
    * @return
    */
    @Select("${sql}")
    List<JSONObject> selectBySQL(@Param("sql") String sql, @Param("et")Map param);

    /**
    * 自定义更新SQL
    * @param sql
    * @return
    */
    @Update("${sql}")
    boolean updateBySQL(@Param("sql") String sql, @Param("et")Map param);

    /**
    * 自定义插入SQL
    * @param sql
    * @return
    */
    @Insert("${sql}")
    boolean insertBySQL(@Param("sql") String sql, @Param("et")Map param);

    /**
    * 自定义删除SQL
    * @param sql
    * @return
    */
    @Delete("${sql}")
    boolean deleteBySQL(@Param("sql") String sql, @Param("et")Map param);


}